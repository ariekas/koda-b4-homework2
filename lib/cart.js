...
async function cart() {
  console.log("\n=== MENU CART ===");
  console.log("1. Lihat Cart Saat Ini");
  console.log("2. Checkout Cart");
  console.log("3. Kembali");

  const choice = await question("Pilih menu (1-3): ");
  ...
  switch (choice) {
    ...
    case "2":
      // Checkout cart
      if (cartItems.length === 0) {
        console.log("\nCart kosong, tidak ada yang bisa di-checkout.");
      } else {
        // Tampilkan cart sebelum checkout
        console.log("\n=== KONFIRMASI CHECKOUT ===\n");
        let grandTotal = 0;
